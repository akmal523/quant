...
def test_classify_asset_tiers():
    ...
    from portfolio import classify_asset
...
def test_calculate_min_trade_size_floor():
    ...
    from optimizer import calculate_min_trade_size
...
def test_passes_fee_hurdle():
    ...
    from optimizer import passes_fee_hurdle
...
def test_should_rebalance_core_drift_threshold():
    ...
    init_db()
    from portfolio import should_rebalance_asset
    # Force a rebalance_log row so time gate passes (last = 100 days ago).
    from portfolio import set_last_rebalance
...
def test_check_volume_liquidity():
    ...
    from optimizer import check_volume_liquidity
    df = pd.DataFrame({
        "Close": [100.0] * 30,
