...
from taxonomy import validate_isin, INVERSE_STRUCTURE
from routing import route_signal, build_execution_instruction
from scoring import etf_quality_score, etf_tactical_grade
from discovery import months_between, demote_stale_active, graduate
...
class TestDelistTracking(unittest.TestCase):
    def test_three_failures_produce_delisted(self):
        ...
        from database import init_db, get_connection
        from taxonomy import mark_delisted
        init_db()
        conn = get_connection()
        conn.execute(
